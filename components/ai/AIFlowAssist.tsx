  ...
  return (
    <>
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-violet-500/40 bg-violet-500/10 text-xs font-medium text-violet-300 hover:bg-violet-500/20 transition-all"
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#202c33] border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/30 placeholder:text-white/30 resize-none"
                />
                <p className="text-[11px] text-white/30 mt-1">Plain language — no technical terms needed.</p>
              </div>

              <div className="w-40">
                <label className="text-xs font-medium text-white/50 uppercase tracking-wide block mb-1.5">Reply language</label>
                <select
                  value={language}
                  onChange={(e) => setLanguage(e.target.value)}
                  className="w-full bg-[#202c33] border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none"
                >
              ...
              {flow && (
                <div className="rounded-lg border border-violet-500/30 bg-violet-950/30 p-3">
                  <div className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-violet-900/50 px-2 py-0.5 text-[11px] font-medium text-violet-300">
                    <Sparkles className="h-3 w-3" /> AI-generated — review before publishing
                  </div>
                  <p className="text-sm text-white/80 font-medium mb-1">{flow.name || "Automation flow"}</p>
                  <p className="text-xs text-white/50 mb-2">{flow.nodes.length} steps · {flow.edges.length} connections</p>
                  <ol className="space-y-1 mb-3 max-h-40 overflow-y-auto">
                    {flow.nodes.map((n) => (
                      <li key={n.id} className="text-xs text-white/60 flex items-start gap-2">
                        <span className="text-violet-400 mt-0.5">•</span>
                        <span><span className="text-white/80">{n.data.label}</span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button onClick={apply} className="rounded-lg bg-violet-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-violet-700">
                      Add to canvas
                    </button>
                    <button
                      onClick={() => generate(true)}
                      disabled={loading}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-sm text-white/70 hover:bg-white/5 disabled:opacity-60"
                    >
