  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage API keys for programmatic access to SendAnjal
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 wa-gradient text-primary-foreground text-sm font-semibold px-4 py-2.5 rounded-xl hover:opacity-90 transition-all"
          >
      ...
      {newKey && (
        <div className="bg-success-soft border border-success/25 rounded-2xl p-5">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-xl bg-success-soft flex items-center justify-center flex-shrink-0">
              <Shield className="w-4.5 h-4.5 text-success" />
            </div>
            <div className="flex-1">
              <p className="font-semibold text-success">Key created — save it now!</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                This key will never be shown again. Copy it to a secure location.
              </p>
              <div className="mt-3 flex items-center gap-2">
                <code className={`flex-1 bg-black/40 border border-border rounded-xl px-3 py-2 text-xs font-mono text-success ${!showRaw ? "blur-[3px] select-none" : ""}`}>
                  {newKey.rawKey}
                </code>
                <button onClick={() => setShowRaw(!showRaw)} className="p-2 rounded-lg bg-secondary hover:bg-secondary transition-colors flex-shrink-0">
                  {showRaw ? <EyeOff className="w-4 h-4 text-muted-foreground" /> : <Eye className="w-4 h-4 text-muted-foreground" />}
                </button>
                <button onClick={copyKey} className="p-2 rounded-lg bg-success-soft hover:bg-success-soft transition-colors flex-shrink-0">
                  {copied ? <Check className="w-4 h-4 text-success" /> : <Copy className="w-4 h-4 text-success" />}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="divide-y divide-border/50">
            {keys.map((k) => (
              <div key={k.id} className="p-4 flex items-center gap-4 hover:bg-muted/20 transition-colors">
                ...
                <button
                  onClick={() => revoke(k.id)}
                  className="p-2 rounded-lg hover:bg-destructive-soft transition-colors flex-shrink-0"
                >
      ...
      <div className="bg-card rounded-2xl border border-border/50 p-5">
        ...
        <pre className="bg-rail rounded-xl p-4 text-xs text-success overflow-x-auto">
{`# Send a WhatsApp message via REST API
curl -X POST https://sendanjal.com/api/v1/messages/send \\
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex items-start gap-2 p-3 bg-warning-soft border border-warning/25 rounded-xl">
                  <AlertTriangle className="w-4 h-4 text-warning flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-warning">The key will be shown only once. Store it securely.</p>
                </div>
              </div>
              <div className="p-5 border-t border-border flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 wa-gradient text-primary-foreground text-sm font-semibold py-2.5 rounded-xl hover:opacity-90 disabled:opacity-40"
                >
