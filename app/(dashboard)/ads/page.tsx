    ...
    return (
      <div className="max-w-5xl">
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-12 text-center">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-accent to-accent flex items-center justify-center mx-auto mb-5 border border-primary/25">
          ...
          <div className="flex items-center justify-center gap-3">
            <button
              ...
              className="flex items-center gap-2 bg-[#1877F2] hover:bg-[#0e64d6] text-white font-semibold px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-primary"
            >
    ...
    <div className="max-w-7xl">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            { label: "Ad Spend",     value: formatINR(summary.spend),         icon: IndianRupee,       color: "text-warning",  bg: "bg-warning-soft" },
            { label: "Leads",        value: fmtNum(summary.leads),            icon: Users,             color: "text-primary",   bg: "bg-accent" },
            { label: "Conversions",  value: `${fmtNum(summary.conversions)}`, icon: Target,            color: "text-success", bg: "bg-success-soft",
              sub: `${summary.conversion_rate}% conv rate` },
            { label: "ROAS",         value: `${summary.roas.toFixed(2)}x`,    icon: TrendingUp,        color: summary.roas >= 1 ? "text-success" : "text-destructive",
              bg: summary.roas >= 1 ? "bg-success-soft" : "bg-destructive-soft",
      ...
      {summary && summary.spend > 0 && (
        <div className="bg-card border border-border/50 rounded-2xl p-5 mb-6">
          ...
          <div className="flex items-end gap-1.5 h-24">
            {[
              { label: "Impressions",  value: summary.impressions, color: "from-accent    to-accent" },
              { label: "Clicks",       value: summary.clicks,      color: "from-accent  to-accent" },
              { label: "Leads",        value: summary.leads,       color: "from-accent to-accent" },
              { label: "Messages",     value: summary.messages,    color: "from-success-soft to-success-soft" },
              { label: "Conversions",  value: summary.conversions, color: "from-warning-soft   to-warning-soft" },
              ...
              return (
                <div key={label} className="flex-1 flex flex-col items-center justify-end gap-2">
                  ...
                  <div
                    className={`w-full bg-gradient-to-t ${color} border-t border-border rounded-t-lg`}
                    style={{ height: `${pct}%` }}
                  />
          ...
          <EmptyState
            ...
            action={
              <button
                onClick={handleSync}
                disabled={syncing}
                className="flex items-center gap-2 wa-gradient text-primary-foreground font-semibold px-5 py-2.5 rounded-xl hover:opacity-90 transition-all"
              >
                ...
                {campaigns.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="border-b border-border/20 last:border-0 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-3 py-3 text-right tabular-nums font-semibold text-primary">{c.leads}</td>
                      <td className="px-3 py-3 text-right tabular-nums text-muted-foreground">{c.messages_sent}</td>
                      <td className="px-3 py-3 text-right tabular-nums">
                        <span className="font-semibold text-success">{c.conversions}</span>
                        {c.leads > 0 && (
      ...
      {/* Info footer */}
      <div className="mt-6 p-4 bg-accent border border-primary/25 rounded-xl flex items-start gap-3">
        ...
        <div className="text-xs text-muted-foreground leading-relaxed">
          ...
          Click-to-WhatsApp ad, Meta sends a referral payload to your webhook. SendAnjal matches it to the campaign,
        ...
        <div className="p-5 space-y-4">
          <div className="bg-accent border border-primary/25 rounded-xl p-3 text-xs text-muted-foreground space-y-1.5">
            ...
            <ol className="list-decimal list-inside space-y-0.5">
              <li>Go to <a href="https://developers.facebook.com/tools/explorer/" target="_blank" rel="noreferrer" className="text-primary hover:underline inline-flex items-center gap-0.5">Graph API Explorer <ExternalLink className="w-3 h-3" /></a></li>
