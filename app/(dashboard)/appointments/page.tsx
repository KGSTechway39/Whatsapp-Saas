...
const STATUS_META: Record<ApptStatus, { label: string; color: string; bg: string; border: string; icon: React.ElementType }> = {
  ...
  confirmed:   { label: "Confirmed",   color: "text-success", bg: "bg-success-soft", border: "border-success/25", icon: CheckCircle2 },
  completed:   { label: "Completed",   color: "text-primary",  bg: "bg-accent",  border: "border-primary/25",  icon: Star },
  cancelled:   { label: "Cancelled",   color: "text-destructive",     bg: "bg-destructive-soft",     border: "border-destructive/25",     icon: XCircle },
  no_show:     { label: "No-show",     color: "text-warning",  bg: "bg-warning-soft",  border: "border-warning/25",  icon: AlertCircle },
  rescheduled: { label: "Rescheduled", color: "text-warning",   bg: "bg-warning-soft",   border: "border-warning/25",   icon: RefreshCw },
};

const SERVICE_META: Record<ServiceType, { label: string; color: string; bg: string }> = {
  consultation: { label: "Consultation", color: "text-primary",    bg: "bg-accent" },
  follow_up:    { label: "Follow-up",    color: "text-success", bg: "bg-success-soft" },
  demo:         { label: "Product Demo", color: "text-primary",  bg: "bg-accent" },
  checkup:      { label: "Check-up",     color: "text-primary",    bg: "bg-accent" },
  meeting:      { label: "Meeting",      color: "text-warning",   bg: "bg-warning-soft" },
  callback:     { label: "Callback",     color: "text-primary",    bg: "bg-accent" },
};
          ...
          <div className="relative">
            ...
            {showMenu && (
              <div className="absolute right-0 top-7 z-20 bg-card border border-border rounded-xl shadow-lg w-44 overflow-hidden">
                ...
                <button onClick={() => setShowMenu(false)} className="w-full text-left px-3 py-2 text-xs text-destructive hover:bg-destructive-soft transition-colors flex items-center gap-2">
      ...
      <div className="flex items-center gap-1.5 flex-wrap pt-2 border-t border-border/30">
        {!appt.confirmationSent && (
          <button onClick={() => onSendWA(appt, "confirm")}
            className="flex items-center gap-1 text-[10px] font-medium px-2.5 py-1.5 rounded-lg bg-success-soft text-success border border-success/25 hover:bg-success-soft transition-colors">
            <CheckCircle2 className="w-3 h-3" /> Confirm
          </button>
        )}
        {appt.confirmationSent && !appt.reminderSent && appt.status !== "cancelled" && (
          <button onClick={() => onSendWA(appt, "reminder")}
            className="flex items-center gap-1 text-[10px] font-medium px-2.5 py-1.5 rounded-lg bg-accent text-primary border border-primary/25 hover:bg-accent transition-colors">
        ...
        {(appt.status === "no_show" || appt.status === "cancelled") && (
          <button onClick={() => onSendWA(appt, "reschedule")}
            className="flex items-center gap-1 text-[10px] font-medium px-2.5 py-1.5 rounded-lg bg-warning-soft text-warning border border-warning/25 hover:bg-warning-soft transition-colors">
            <RefreshCw className="w-3 h-3" /> Reschedule
          </button>
        )}
        {/* Status indicators */}
        <div className="flex items-center gap-1.5 ml-auto">
          {appt.confirmationSent && <span title="Confirmation sent" className="w-4 h-4 rounded-full bg-success-soft flex items-center justify-center"><Check className="w-2.5 h-2.5 text-success" /></span>}
          {appt.reminderSent && <span title="Reminder sent" className="w-4 h-4 rounded-full bg-accent flex items-center justify-center"><Bell className="w-2.5 h-2.5 text-primary" /></span>}
          {appt.followUpSent && <span title="Follow-up sent" className="w-4 h-4 rounded-full bg-accent flex items-center justify-center"><MessageSquare className="w-2.5 h-2.5 text-primary" /></span>}
        </div>
      </div>
    </div>
  );
          ...
          return (
            <button
              ...
              className={`relative w-7 h-7 rounded-lg text-xs flex items-center justify-center mx-auto transition-all ${
                isSelected ? "wa-gradient text-primary-foreground font-bold shadow" :
    ...
    <div className="max-w-7xl">
        ...
        action={
          <div className="flex items-center gap-2">
            <Link href="/appointments/automations"
              ...
              <Zap className="w-4 h-4 text-primary" /> Automations
            </Link>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        {[
          ...
          { label: "Upcoming",   value: upcoming,            color: "text-primary",       bg: "bg-accent",       icon: Clock },
          { label: "Confirmed",  value: confirmed,           color: "text-success",    bg: "bg-success-soft",    icon: CheckCircle2 },
          { label: "Completed",  value: completed,           color: "text-primary",     bg: "bg-accent",     icon: Star },
          { label: "No-shows",   value: noShows,             color: "text-warning",     bg: "bg-warning-soft",     icon: AlertCircle },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-accent border border-primary/25 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <Zap className="w-4 h-4 text-primary" />
              <p className="text-xs font-semibold text-primary">Quick Automations</p>
            </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {loading ? (
            ...
            <div className="bg-card rounded-2xl border-2 border-dashed border-border/50 p-16 text-center">
              ...
              <Link href="/appointments/book"
                className="mt-4 inline-flex items-center gap-2 wa-gradient text-primary-foreground text-sm font-semibold px-5 py-2.5 rounded-xl hover:opacity-90 transition-all">
                <Plus className="w-4 h-4" /> Book Appointment
              </Link>
            </div>
          ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-fade-in">
            ...
            <div className="bg-muted/20 rounded-xl p-4 mb-4">
              ...
              <div className="bg-chat-ground rounded-xl p-3">
                <div className="bg-chat-in border border-chat-inBorder rounded-xl rounded-tl-none p-3">
                  ...
                  <p className="text-[9px] text-muted-foreground text-right mt-1">12:30 ✓✓</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmSendWA} className="flex-1 flex items-center justify-center gap-2 wa-gradient text-primary-foreground text-sm font-semibold px-4 py-2.5 rounded-xl hover:opacity-90 transition-all">
